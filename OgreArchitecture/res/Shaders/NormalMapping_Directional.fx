...
VS_OUTPUT NormalMapping_Pass_0_Vertex_Shader_vs_main( VS_INPUT Input )
{
   ...
   float3 lightDir = gWorldLightPosition;
   float3 viewDir = normalize(worldPosition - gCameraPosition);
   
   Output.lightDir = -lightDir;
   Output.viewDir = viewDir;
   
   float3 worldNormal = normalize(mul( Input.normal, gWorldMatrix ));
   float3 worldTangent = normalize(mul( Input.tangent, gWorldMatrix));
   float3 worldBinormal = normalize(mul( Input.binormal, gWorldMatrix));
...
<
   string ResourceName = "C:\\Users\\Tipp7\\Desktop\\ShaderPrimerKR-master\\05_DiffuseSpecularMapping\\fieldstone_SM.tga";
>;
...
float3 gLightAmbient
<
   string UIName = "gLightAmbient";
...
float4 NormalMapping_Pass_0_Pixel_Shader_ps_main ( PS_INPUT Input ) : COLOR
{
   ...
   if(diffuse.x > 0)
   {
      ...
      specular = gLightColor * specularIntensity * specular;
   }
   float3 ambient = gLightAmbient * albedo;
