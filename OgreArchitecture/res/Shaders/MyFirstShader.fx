...
struct VS_OUTPUT
{
   float4 mPosition : POSITION; //ÀX  X
...
VS_OUTPUT Default_DirectX_Effect_Pass_0_Vertex_Shader_vs_main( VS_INPUT Input )
{
   ...
   Output.mPosition = mul( Input.mPosition, gWorldMatrix ); // ÔÜ ÀX
   ...
   Output.mPosition = mul( Output.mPosition, gViewMatrix ); //  ?ÀX
   Output.mPosition = mul( Output.mPosition, gProjectionMatrix ); //  , ÀX

   float3 worldNormal = mul( Input.mNormal, (float3x3)gWorldMatrix ); //x@ ? ÔÜ ÀX
...
float4 Default_DirectX_Effect_Pass_0_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   float3 ambient = float3(0.1f, 0.1f, 0.1f); //üÀ
